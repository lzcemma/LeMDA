    ...
    def _fit(
        ...
        **hpo_kwargs,
        ...
        if hasattr(config, MATCHER):
            warnings.warn(
                "Matching is experimental. We may change its behaviors in future.",
                UserWarning,
            )
            match_label = self._df_preprocessor.label_generator.transform(
                [self._config.matcher.match_label]
            ).item()
        ...
        assert not (
            is_distill and is_match
        ), "Can't do distillation and matching simultaneously"
        if is_distill:
