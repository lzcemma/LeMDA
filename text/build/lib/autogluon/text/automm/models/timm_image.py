...
class TimmAutoModelForImagePrediction(nn.Module):
    ...
    def __init__(
        ...
        self.head_layer_names = [
            n for n, layer_id in self.name_to_id.items() if layer_id == 0
        ]
    ...
    def forward(
        ...
        if self.mix_choice == "all_images":  # mix inputs
            mixed_images = (
                images.sum(dim=1) / image_valid_num[:, None, None, None]
            )  # mixed shape: (b, 3, h, w)
            ...
            image_masks = (
                steps.reshape((1, -1)) < image_valid_num.reshape((-1, 1))
            ).type_as(
                logits
            )  # (b, n)
            features = (
                features.reshape((b, n, -1)) * image_masks[:, :, None]
            )  # (b, n, num_features)
            logits = (
                logits.reshape((b, n, -1)) * image_masks[:, :, None]
            )  # (b, n, num_classes)
